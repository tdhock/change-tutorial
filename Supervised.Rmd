...
* overall the optimization problem is non-convex, which means that
  gradient-based algorithms can't be used to compute the exact
  solution. However the dynamic dynamic programming algorithms
  discussed above can be used to compute the solution in a reasonable 

A few references about the `Segmentor` function:
...
model with the feature $\mathbf x_i=\log\log d_i$ -- this is the column
`n.loglog` of the feature matrix.
$$f(\mathbf x_i) = \log\lambda_i = \mathbf x_i = \log\log d_i$$
...
using the same log.log.n feature. The survreg function solves the
following optimization problem
$$ 
\operatorname{minimize}_{
	\mathbf w\in\mathbb R^p, \beta\in\mathbb R,\sigma\in\mathbb R_+
	}
\sum_{i=1}^n -\log[
...
* the optimization variables are the coefficients or weight vector
  $\mathbf w\in\mathbb R^p$, the bias/intercept $\beta\in\mathbb
  R$, and the scale $\sigma\in\mathbb R_+$.
...
* $\log\lambda_i= f(\mathbf x_i)=\mathbf w^\intercal \mathbf
  x_i+\beta$ is the predicted log(penalty) value for data sequence $i$.
...
**Exercise:** try re-doing the analyses in this section using
...
$$ 
\operatorname{minimize}_{
	\mathbf w\in\mathbb R^p, \beta\in\mathbb R
	}
	\gamma ||\mathbf w||_1 + 
\sum_{i=1}^n 
...
* the hyper-parameter $\gamma>0$ is the degree of L1-regularization
  (chosen via an internal cross-validation loop).
* the L1-norm $||\mathbf w||_1=\sum_{j=1}^p |w_j|$ forces some of the
  $w_j$ weights to be exactly equal to zero (automatic feature
  selection).
...
features. Before training, we register a foreach parallel backend (the
internal cross-validation loop to choose the regularization parameter
...
and IntervalRegressionCV is slightly more accurate than survreg. The
ROC curves are plotted below.
...
breakpoint/positive regions with no predicted changepoints).
