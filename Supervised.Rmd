...
changepoint models using the `Segmentor` function. First a few
references about the `Segmentor` function:

* it solves the "Segment Neighborhood" problem originally described by
  [Auger and Lawrence (1989)](https://www.ncbi.nlm.nih.gov/pubmed/2706400). They
  proposed an $O(S_{\text{max}} n^2)$ algorithm for computing the most
  likely models with $s\in\{1,\dots,S_{\text{max}}\}$ segments, for
  $n$ data points. An R implementation of their algorithm is available
  as `changepoint::cpt.mean(method="SegNeigh")` but it is very slow
  for large $n$, due to the quadratic time complexity.
* [Rigaill (2010) discovered a Pruned Dynamic Programming Algorithm (PDPA)](https://arxiv.org/abs/1004.0887)
  ...
  $O(S_{\text{max}}n\log n)$ time complexity makes it much faster for
...
* [Cleynen et al (2014) generalized the PDPA to other loss/likelihood models](https://almob.biomedcentral.com/articles/10.1186/1748-7188-9-6). Their
  R function `Segmentor3IsBack::Segmentor` implements the $O(S_{\text{max}}n\log n)$
  PDPA for Poisson, Normal homoscedastic, Negative Binomial, Normal
  Heteroscedastic, and Exponential likelihood models.
...
code below. Then create the rest of the figures in this section using
your labeled data.
...
is the loss (sum of squared residuals) of the model with $s\in\{1,\dots,s_{\text{max}}\}$
...
**Question:** why are there only 

Another way to visualize the predicted penalty values is with the error
function plot below.
...
**Exercise:** can you think of any other features which would be
useful for predicting log(penalty) values? In other words, what
...
The plot above also shows the surrogate loss that `survreg` minimizes
...
**Exercise:** the survreg model above assumed a Gaussian distribution
for the log(penalty) values. Would using another distribution increase
accuracy? Hint: try `survreg(dist="logistic")`.
...
regularization. This is a machine learning technique that
...
features. Next, we use `IntervalRegressionCV` to fit the regularized
linear model. If a foreach parallel backend is registered, the
internal cross-validation loop is performed in parallel. We use
`set.seed` in order to make the results reproducible (there is
randomness in the fold assignment in the internal cross-validation
loop).
...
trained using 256 possible input feature, it has selected only a few
...
six.cv.surrogate <- six.survreg.res[, {
  ...
  data.table(
    ...
    log.penalty=log.pen.vec,
    surrogate.loss=squared.hinge(
      log.pen.vec-min.log.lambda)+squared.hinge(
        max.log.lambda-log.pen.vec)
...
accuracy of the learned models. In fact, we have already
**qualitatively** evaluated the accuracy of the predictions by visual
inspection of the plots in the last section. However because we have
labels, we can also **quantitatively** evaluate the prediction
accuracy by computing the number of incorrect labels and AUC (area
under the ROC curve).
...
if(!file.exists("Segmentor.models.RData")){
  download.file(
    "http://members.cbio.ensmp.fr/~thocking/data/Segmentor.models.RData",
...
It is clear that survreg is significantly more accurate than BIC/SIC,
and IntervalRegressionCV is slightly more accurate. The ROC curves are
plotted below.
...
pattern, supervised learning algorithms can be used to predict penalty
values and changepoints which are more accurate than unsupervised
methods (with respect to the labels).
