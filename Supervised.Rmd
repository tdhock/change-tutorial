...
**Exercise after class:** plot data and labels for a different set of
profiles. Hint: assign a new value to `some.ids`, then re-run the code
above. 

## Creating labels via visual inspection

**Exercise after class:** create a set of labels via visual
inspection, by following the instructions in this section. 
...
Then make a data.frame with the label type and positions, as below.
...
Note that we used the "1change" label above, which means that the
model should predict exactly 1 changepoint in the region (0 predicted
changes is a false negative, and 2 or more is a false positive).
...
**Exercise during class:** try changing the `pen.name` variable
above to another theretically-motivated unsupervised penalty. Other
options from `help(cpt.mean)` are SIC (without a zero), MBIC, AIC, and
...
Gaussian models with $s\in\{1,\dots,5\}$ segments. In contrast, a
...
It is clear from the plot above that there is a subset of models which
is consistent with the label. The goal of a supervised changepoint
detection algorithm is to predict one of those models.
...
* complexity, such as number of changepoints or segments. This column
...
the larger the penalty, the fewer the number of changepoints (and vice
...
log(penalty) values that achieves minimum incorrect labels. Now that
...
sequence $i$ and every number of segments $s$, we can visualize the
...
`problem.vars` argument indicates the data sequence ID columns. The function 
...
classification, for which the vertical axis is a second feature. The
plot below uses a `geom_segment` to emphasize the fact that the target
intervals are defined in terms of log(penalty) values (the vertical
axis).
...
useful for showing the errors regression plots such as the one below.
...
**Exercise during class:** In the changepoint package version 2.2
...
compute the optimal changepoints for each of the 36 labeled
neuroblastoma data sequences we plotted above. Verify that the penalty
and changepoints are the same as we computed above, by adding a
`geom_point` for predicted log(penalty) values to the plot with the
regression line above. Hint: when we computed `changepoint::cpt.mean`
in the first part of the tutorial, we saved the predicted penalty in
the `pen.value` column of `unsupervised.models`. Join that data.table
with `six.features.targets` to get the predicted penalty for the
...
**Exercise during class:** the survreg model above assumed a Gaussian distribution
...
`survreg(dist="logistic")` model. For other choices, see `help(survreg)`.
...
inputs are a numeric feature matrix (n observations x p features), and
a numeric target matrix (n observations x 2). It outputs a model fit
...
for(test.fold in 1:n.folds){
  ...
  ## fit.margin <- penaltyLearning::IntervalRegressionCVmargin(
  ##   train.features[, some.feature.names], train.target.mat)
  ...
  pred.list <- list(
    ...
    ##IntervalRegressionCVmargin=predict(fit.margin, labeled.feature.mat[test.id.vec,]),
...
AUC. Below we compute and plot two accuracy metrics, AUC and percent
incorrectly predicted labels.
