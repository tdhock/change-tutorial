...
model with $\beta=0$ and $w=1$ for the feature $\mathbf x_i=\log\log
d_i$ -- this is the column `n.loglog` of the feature matrix.
$$\log\lambda_i = f(\mathbf x_i) = \beta + w\log\log d_i$$ This means
the penalty is $\lambda_i=e^\beta(\log d_i)^w$ in the "optimal
partitioning" version of the segmentation problem, $$
\operatorname{minimize}_{\mathbf m\in\mathbb R^{d_i}} \sum_{j=1}^{d_i}
\ell(z_{ij}, m_j) +e^\beta(\log d_i)^w \sum_{j=1}^{d_i-1} I(m_{j} \neq
m_{j+1}).  $$
...
f(\mathbf x_i) = \beta + \mathbf w^\intercal x_i$ 
