...
six.segs <- Segmentor.models$segs[profile.id %in% some.ids]
six.selection <- Segmentor.models$loss[profile.id %in% some.ids, {
  penaltyLearning::modelSelection(.SD, complexity="n.segments")
}, by=list(profile.id, chromosome)]
six.error.list <- penaltyLearning::labelError(
  six.selection, six.labels, six.segs[1 < start],
  problem.vars=c("profile.id", "chromosome"))
...
sequence $i$. Thus in this framework we can think of the BIC/SIC
criterion as a linear model with the feature $x_i=\log\log n_i$.
$$f(x_i) = \log\lambda_i = 0 + 1*\log\log n_i$$

```{r}
six.BIC.dt <- data.table::data.table(six.profiles)[, list(
  log.log.n=log(log(.N))
  ), by=list(profile.id, chromosome)]
```
