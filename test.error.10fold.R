...
model.fun.list <- list(
  BIC=function(){
    ...
    IntervalRegressionUnregularized(
      train.feature.mat[, c("log.n"), drop=FALSE], train.target.mat)
  })
...
set.seed(1)
test.error.10fold.list <- list()
## 10-fold CV by simple random sampling.
test.fold.vec <- paste(sample(rep(1:10, l=nrow(target.dt))))
for(test.fold in unique(test.fold.vec)){
  ...
  for(model.name in names(model.fun.list)){
    ...
    test.error.10fold.list[[paste(test.fold, model.name)]] <- model.test.error
  }
}
test.error.10fold <- do.call(rbind, test.error.10fold.list)

save(test.error.10fold, file="test.error.10fold.RData")
